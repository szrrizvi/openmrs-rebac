...
public class PatientState extends BaseOpenmrsData implements java.io.Serializable, Comparable<PatientState> {
	...
	/**
	 * Returns a readable name for the patient state.
	 * Program - ProgramWorkflow - ProgramWorkflowState
	 */
	public String getPatientStateInfo() {
		
		String stateName = state.getConcept().getName().getName();
		String workflowName = state.getProgramWorkflow().getConcept().getName().getName();
		String programName = state.getProgramWorkflow().getProgram().getName();
		
		String str = programName + " - " + workflowName + " - " + stateName;
	...
	public List<Person> getResourceOwners() {
		...
		owners.add(patientProgram.getPatient());
		return owners;
	}
}
